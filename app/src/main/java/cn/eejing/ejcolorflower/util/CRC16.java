...
public class CRC16 {
    private final static int[] crc_const_table = new int[]{
    ...
    public static int calculate(byte[] buffer, int cnt) {
        int crc = 0xffff;
        int by;
        int ddd;
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            by = (crc >> 8) & 0xff;
            crc = (crc) << 8;
            crc = (crc ^ crc_const_table[(ddd ^ by) & 0xff]);
        }
        return crc & 0xffff;
    }

    public static boolean validate(byte[] buffer, int data_len) {
        int crc = calculate(buffer, data_len);
        return buffer[data_len + 1] == (byte) (crc >> 8) &&
                buffer[data_len] == (byte) (crc & 0xff);
    }
